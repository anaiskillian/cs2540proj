...
def encode_nn_in_z3(model, input_vars):
    """Encode the FeedForwardNeuralNetwork as Z3 constraints."""
    constraints = []
    ...
    for i, layer in enumerate(model.layers):
        weights = layer.weight.detach().numpy()  # shape: [out, in]
        biases = layer.bias.detach().numpy()     # shape: [out]
        layer_out = []
    ...
    # Process output layer (no ReLU)
    output_weights = model.output_layer.weight.detach().numpy()
    output_biases = model.output_layer.bias.detach().numpy()
    final_output = []

    for j in range(len(output_biases)):
        weighted_sum = output_biases[j]
        for k in range(len(current_vars)):
            weighted_sum += output_weights[j][k] * current_vars[k]

        output_var = Real(f'output_{j}')
        constraints.append(output_var == weighted_sum)
        final_output.append(output_var)
    
    return constraints, final_output
def verify_robustness(model, x_sample, epsilon, expected_class, num_classes):
    ...
    if result == sat:
        ...
        print("Perturbed Outputs ", sigmoid(perturbed_outputs[0]))
...
def adversarial_attack(model, x_sample, epsilon, expected_class, steps=100):
    ...
    target = torch.FloatTensor([[expected_class]])
...
def set_the_model(): 
    ...
    target_output = np.array([[1], [0], [1], [0], [0], [0], [1], [0], [0], [0], [0], [0]])  # Pass or fail (1 = pass, 0 = fail)
...
def main(): 
    ...
    input = torch.tensor([4,7], dtype=torch.float32)
